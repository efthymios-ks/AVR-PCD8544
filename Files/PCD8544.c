...
static inline void Pulse_SCLK(void);
//---------------------------------------------//
...
{
	//Enable data
	DigitalWrite(GLCD_DC, High);
	
	//Send data
	GLCD_Send(__GLCD.Mode == GLCD_Non_Inverted ? Data : ~Data);
		
	if (__GLCD.X < (__GLCD_Screen_Width - 1))
		__GLCD.X++;
}
...
void GLCD_Setup(void)
{
	//Setup pins
	//GLCD pins = Outputs
	PinMode(GLCD_SCE, Output);
	PinMode(GLCD_RST, Output);
	PinMode(GLCD_DC, Output);
	PinMode(GLCD_DIN, Output);
	PinMode(GLCD_SCLK, Output);
	
	//GLCD pins = 1 (Disabled)
	DigitalWrite(GLCD_DC, Low);
	DigitalWrite(GLCD_DIN, Low);
	DigitalWrite(GLCD_SCLK, Low);
	
	//Reset display
	DigitalWrite(GLCD_RST, High);
	DigitalWrite(GLCD_SCE, High);
	_delay_ms(1);
	...
	//Initialize chip
	//Enable controller
	DigitalWrite(GLCD_SCE, Low);
	//Display Active, Horizontal Addressing, Extended Instruction Set
	GLCD_SendCommand(__GLCD_Command_Function_Set | __GLCD_Command_On | __GLCD_Command_Horizontal_Addressing | __GLCD_Command_Extented_Instruction_Set);
	//Bias Mode 1:48 (1:48 is the optimum based on the datasheet)
	GLCD_SendCommand(__GLCD_Command_Bias_System | __GLCD_Command_MUX_Rate_1_48);
	//Set temperature coefficient
	GLCD_SendCommand(__GLCD_Command_Temperature_Control | __GLCD_Command_VLCD_Temperature_Coefficient_2);
	//Set Vop (optimum for 1:48 = 3.06 + 66 * 0.06 = 7V)
	GLCD_SendCommand(__GLCD_Command_Set_Top | 0x42);
	//Basic Instruction Set
	GLCD_SendCommand(__GLCD_Command_Function_Set | __GLCD_Command_On | __GLCD_Command_Horizontal_Addressing | __GLCD_Command_Basic_Instruction_Set);
	//All display segments on
	GLCD_SendCommand(__GLCD_Command_Display_Control | __GLCD_Command_All_Display_Segments_On);
	//Display blank
	GLCD_SendCommand(__GLCD_Command_Display_Control | __GLCD_Command_Display_Blank);
	//Normal Mode
	GLCD_SendCommand(__GLCD_Command_Display_Control | __GLCD_Command_Normal_Mode);
...
void GLCD_Render(void)
{
	...
	for (j = 0 ; j < __GLCD_Screen_Height ; j += __GLCD_Screen_Line_Height)
	{
		...
		for (i = 0 ; i < __GLCD_Screen_Width ; i++)
			GLCD_Data(GLCD_BufferRead(i, __GLCD.Y));
	}
}
...
static void GLCD_Send(const uint8_t Data)
{
	...
	//Send byte using Bit-Bang method
	for (i = 7 ; i >= 0 ; i--)
	{
		//DIN = BIT
		DigitalWrite(GLCD_DIN, BitCheck(Data, i));
...
static void __GLCD_GotoX(const uint8_t X)
{
	if (X < __GLCD_Screen_Width)
	{
		...
		//Update command
		cmd = __GLCD_Command_Set_X_Address | X;
...
static void __GLCD_GotoY(const uint8_t Y)
{
	if (Y < __GLCD_Screen_Height)
	{
		...
		//Update command
		cmd = __GLCD_Command_Set_Y_Address | (Y / __GLCD_Screen_Line_Height);
